...
class Usage(Exception):
    ...
    def show_help(self):
        usage = ['%s' % '']
        usage.append('%s' % 'usage: crams-provision-nectar '
        ...
        usage.append('%s' % '    crams-provision-nectar     '
                            'without any arguments. provision all '
                            'allocation requests and exit')
        usage.append('%s' % '')
        usage.append('%s' % 'Examples:')
        usage.append('%s' % '  1):')
        usage.append('%s' % '       crams-provision-nectar -l       '
                            'list all allocations ids')

        usage.append('%s' % '')
        usage.append('%s' % '  2):')
        usage.append('%s' % '       crams-provision-nectar -l -d    '
                            'list all allocations ids in debug mode')
        usage.append('%s' % '')
        usage.append('%s' % '  3):')
        usage.append('%s' % '       crams-provision-nectar 6        '
                            'provision allocation request id 6')

        usage.append('%s' % '')
        usage.append('%s' % '  4):')

        usage.append('%s' % '       crams-provision-nectar -d  8    '
                            'provision allocation request id 8 in debug mode')
        usage.append('%s' % '')
        ...
        try:
            ...
            for opt, o_value in opts:
                if opt in ('-d', '--debug'):
                    ...
                    settings.LOGGING_CONF['handlers']['console'][
                        'level'] = 'DEBUG'
                    settings.LOGGING_CONF['root']['level'] = 'DEBUG'
                    settings.LOGGING_CONF['loggers']['crams_provision'][
                        'level'] = 'DEBUG'
